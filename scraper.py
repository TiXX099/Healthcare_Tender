...
def fetch_etimad_tenders():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            cards = soup.find_all('div', class_='card-body', limit=5)
            for card in cards:
...
def fetch_tanafus_tenders():
    ...
    try:
        ...
        if res.status_code == 200:
            soup = BeautifulSoup(res.text, 'html.parser')
            cards = soup.find_all(['div', 'a'], class_=lambda c: c and 'tender' in c.lower(), limit=4)
            for card in cards:
...
def fetch_ncp_tenders():
    """4. المركز الوطني للتخصيص (الشراكات والمشاريع الصحية)"""
...
def fetch_tenders(url=None):
    """تجميع كافة المناقصات الفعلية من البوابات المباشرة فقط"""
    all_results = []
...
if __name__ == "__main__":
    ...
    print(f"تم جلب {len(results)} مناقصة حقيقية ومباشرة:")
    for res in results:
        print("---")
        print(res)
